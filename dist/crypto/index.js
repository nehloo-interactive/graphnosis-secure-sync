...
export async function deriveKey(passphrase, salt) {
    ...
    const key = sodium.crypto_pwhash(sodium.crypto_secretstream_xchacha20poly1305_KEYBYTES, passphrase, s, opslimit, memlimit, sodium.crypto_pwhash_ALG_ARGON2ID13);
...
export async function encrypt(plaintext, key, salt) {
    ...
    for (let offset = 0; offset < plaintext.length; offset += chunkSize) {
        ...
        const ct = sodium.crypto_secretstream_xchacha20poly1305_push(state, plaintext.subarray(offset, end), null, tag);
...
export async function decrypt(ciphertext, passphraseOrKey) {
    await init();
    if (!startsWith(ciphertext, MAGIC))
        throw new Error('Not a Graphnosis App encrypted blob');
